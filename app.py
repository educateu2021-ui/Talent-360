...
# ---------- DATABASE ----------
# Using a final production filename
DB_FILE = "portal_prod_v18.db"

def init_db():
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
    
    # --- CRITICAL FIX: DROP TABLES TO ENSURE SCHEMA MATCHES CODE ---
    # This prevents the "OperationalError" by removing old mismatched tables
    c.execute("DROP TABLE IF EXISTS tasks_v2")
    c.execute("DROP TABLE IF EXISTS training_repo")
    c.execute("DROP TABLE IF EXISTS training_progress")
    c.execute("DROP TABLE IF EXISTS resource_tracker_v2")
    
    # 1. KPI Table
    c.execute('''CREATE TABLE tasks_v2 (
