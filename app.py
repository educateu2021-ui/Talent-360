...
# ---------- DATABASE ----------
# RENAMED DB TO FLUSH ALL OLD DATA
DB_FILE = "portal_data_production_v1.db"
...
def init_db():
    ...
    # 3. RESOURCE TRACKER TABLE - NEW NAME "v3_final" TO AVOID CONFLICTS
    c.execute('''CREATE TABLE IF NOT EXISTS resource_tracker_v3_final (
        id TEXT PRIMARY KEY,
        employee_name TEXT,
        employee_id TEXT,
        dev_code TEXT,
    ...
    c.execute("SELECT count(*) FROM resource_tracker_v3_final")
    if c.fetchone()[0] == 0:
        # 17 Columns Total (ID + 16 Fields)
        resources = [
        ...
        c.executemany("INSERT INTO resource_tracker_v3_final VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", resources)
...
def get_resource_list():
    conn = sqlite3.connect(DB_FILE)
    try: df = pd.read_sql_query("SELECT * FROM resource_tracker_v3_final", conn)
    except: df = pd.DataFrame()
    conn.close()
    return df
...
def save_resource_entry(data, res_id=None):
    ...
    if res_id:
        set_clause = ", ".join([f"{col}=?" for col in cols])
        c.execute(f"UPDATE resource_tracker_v3_final SET {set_clause} WHERE id=?", (*vals, res_id))
    else:
        new_id = str(uuid.uuid4())[:8]
        placeholders = ",".join(["?"] * (len(cols) + 1))
        c.execute(f"INSERT INTO resource_tracker_v3_final VALUES ({placeholders})", (new_id, *vals))
    conn.commit(); conn.close()
