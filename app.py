...
def app_home():
    ...
    # Custom 2-Grid Logic:
    # Instead of st.columns(4), we creates 2 rows with 2 columns each.
    # On Desktop: This looks like a 2x2 grid.
    # On Mobile: This KEEPS the 2x2 grid (side by side) instead of stacking 1 by 1.
...
def app_kpi():
    ...
    if st.session_state['role'] == "Team Leader":
        ...
        # Stack metrics on mobile for readability
        ...
        with st.expander("➕ Create New Task"):
            with st.form("new_task"):
                ...
                # Mobile friendly stacking inputs
                c_a, c_b = st.columns(2)
                pilot = c_a.selectbox("Assignee", [u['name'] for k,u in USERS.items() if u['role']=="Team Member"])
                comm_date = c_b.date_input("Deadline", min_value=date.today())
            ...
            for _, row in my_tasks.iterrows():
                with st.expander(f"{row['task_name']} ({row['status']})"):
                    ...
                    with st.form(f"upd_{row['id']}"):
                        ...
                        ad = c_b.date_input("Done Date", value=date.today())
