...
# ---------- DATABASE ----------
# Updated version to ensure new schema loads correctly
...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS resource_tracker_v3 (
        id TEXT PRIMARY KEY,
        employee_name TEXT,
        employee_id TEXT,
        dev_code TEXT,
    ...
    c.execute("SELECT count(*) FROM resource_tracker_v3")
    if c.fetchone()[0] == 0:
        # Added dummy cost data to demo entries
        resources = [
        ...
        c.executemany("INSERT INTO resource_tracker_v3 VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", resources)

    # KPI 20 TASKS DEMO DATA (Generated via loop)
    ...
    if c.fetchone()[0] == 0:
        ...
        for i in range(1, 21):
            ...
            otd = "N/A"
            if status == "Completed":
                # Randomize if it was late or on time
            ...
            ftr = "Yes" if random.random() > 0.3 else "No" # 70% chance of Yes
...
def get_resource_list():
    conn = sqlite3.connect(DB_FILE)
    try: df = pd.read_sql_query("SELECT * FROM resource_tracker_v3", conn)
    except: df = pd.DataFrame()
    conn.close()
    return df
...
def save_resource_entry(data, res_id=None):
    ...
    if res_id:
        set_clause = ", ".join([f"{col}=?" for col in cols])
        c.execute(f"UPDATE resource_tracker_v3 SET {set_clause} WHERE id=?", (*vals, res_id))
    else:
        new_id = str(uuid.uuid4())[:8]
        placeholders = ",".join(["?"] * (len(cols) + 1))
        c.execute(f"INSERT INTO resource_tracker_v3 VALUES ({placeholders})", (new_id, *vals))
    conn.commit(); conn.close()

def delete_resource_entry(res_id):
    conn = sqlite3.connect(DB_FILE)
    conn.execute("DELETE FROM resource_tracker_v3 WHERE id=?", (res_id,))
    conn.commit(); conn.close()
...
def get_donut(df):
    if df.empty: return go.Figure()
    # Using FTR for donut instead of status completion
